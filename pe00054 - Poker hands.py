...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 15 23:17:25 2020
...
1: High Card: Highest value card.                                               Done
2: One Pair: Two cards of the same value.                                       Done
3: Two Pairs: Two different pairs.                                              Done
4: Three of a Kind: Three cards of the same value.                              Done
5: Straight: All cards are consecutive values.                                  Done
6: Flush: All cards of the same suit.                                           Done
7: Full House: Three of a kind and a pair.                                      Done
8: Four of a Kind: Four cards of the same value.                                Done
9: Straight Flush: All cards are consecutive values of same suit.               Done
10: Royal Flush: Ten, Jack, Queen, King, Ace, in same suit.                     Done
...
example, both players have a pair of queens, then highest cards in each hand are compared (see example 4 below); 
...
def player_1_hand(Hand):
    player1_card = "".join(str(Hand[i]) for i in range(0,14,3))
    player1_suite = "".join(str(Hand[i]) for i in range(1,14,3))
    return [[player1_card, player1_suite]]

def player_2_hand(Hand):
    player2_card = "".join(str(Hand[i]) for i in range(15,30,3))
    player2_suite = "".join(str(Hand[i]) for i in range(16,30,3))
    return [[player2_card, player2_suite]]
       
def hand_rank(player_hands2):
    player_1_info = player_hands2[0]
    
    if len(set(player_1_info[0])) == 4: #If set has 4 elements there must be one with double     
        temp_list = list(set(player_1_info[0]))
        for i in range(len(temp_list)):
            if player_1_info[0].count(temp_list[i]) == 2:
                return [2, value(temp_list[i])]
                    
    elif len(set(player_1_info[0])) == 3: #If set has 3 elements there must be: 2 doubles or 1 triple    
        temp_list = list(set(player_1_info[0]))
        doubles = []
        for i in range(len(temp_list)):
            if player_1_info[0].count(temp_list[i]) == 3:
                return [4, value(temp_list[i])]
            
            elif player_1_info[0].count(temp_list[i]) == 2:
                doubles.append(value(temp_list[i]))
                    
        return [3, set(doubles)]
    
    elif len(set(player_1_info[0])) == 2: #If set has 2 elements there must be: 1 double + 1 Triple or 1 quadruple    
        temp_list = list(set(player_1_info[0]))
        fullhouse = []
        for i in range(len(temp_list)):
            if player_1_info[0].count(temp_list[i]) == 4:
                return [8, value(temp_list[i])]
            
            elif player_1_info[0].count(temp_list[i]) == 3:
                fullhouse.insert(0,value(temp_list[i]))
                
            elif player_1_info[0].count(temp_list[i]) == 2:
                fullhouse.insert(1, value(temp_list[i]))
        return [7, set(fullhouse)]
    
    elif len(set(player_1_info[0])) == 5: #If set has 5 elements we look for straights or flush
        flush = False
        straight = False
        
        if len(set(player_1_info[1])) == 1:
            flush = True
        
        temp_list = []
        for x in range(len(list(player_1_info[0]))):
            temp_list.append(value(player_1_info[0][x]))
            
        temp_list = sorted(temp_list)
        if temp_list[4] - temp_list[3] == 1 and temp_list[3] - temp_list[2] == 1 and temp_list[2] - temp_list[1] == 1\
            and temp_list[1] - temp_list[0] == 1:
                straight = True
        
        if flush == False and straight == False:
            return [1, max(temp_list)]
        
        if straight == False and flush == True:
            temp_var = list(set(player_1_info[1]))
            return [6, suite_value(temp_var[0])]
        
        if straight == True and flush == False:
            return [5, max(temp_list)]
        
        if straight == True and flush == True:
            if max(temp_list) == 14:
                temp_var = list(set(player_1_info[1]))
                return [10, suite_value(temp_var[0])]
            else:
                return [9, max(temp_list)]
    
def compute():
    count = 0
    for x in range(len(HANDS)):
        player_1 = player_1_hand(HANDS[x])
        player_2 = player_2_hand(HANDS[x])
        
        player_1_handrank = hand_rank(player_1)
        player_2_handrank = hand_rank(player_2)
        
        if player_1_handrank[0] > player_2_handrank[0]:
            count += 1
        elif player_1_handrank[0] == player_2_handrank[0]:
            if player_1_handrank[1] > player_2_handrank[1]:
                count += 1
    return count
