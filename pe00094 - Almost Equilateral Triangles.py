...
def compute2(limit): #Actually it turned out to be pretty much the same speed ... so lets go one step further
    
    total = 0
    k = 1
    while True:
        if k > math.sqrt(((limit)/2 - 1)**2 - 1):
            break
...
def compute(limit):
    ...
    while True:
        ...
        C = (((7 - 4*math.sqrt(3))**n + (7 + 4*math.sqrt(3))**n + 1)/3) 
        #Integer solutions of 3C^2 - 2C - 1 - 4k^2 = 0, 2A = C + 1
        
        C1 = (((2 + math.sqrt(3)) * (7 - 4*math.sqrt(3))**(n+1) + (2 - math.sqrt(3))*(7 + 4*math.sqrt(3))**(n+1) - 1)/3) 
        #Integer solutions of 3C^2 + 2C - 1 - 4k^2 = 0, 2A = C - 1)
